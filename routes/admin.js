...
router.get("/", userlogin, async function (req, res, next) {
  ...
  if (role === "admin") {
    ...
    var totViews = await Video.find();
    let totalViews = 0;
    totViews.forEach((Video) => {
  ...
  async (req, res) => {
    try {
      ...
      const videoPath = req.files["video"]
        ? `uploads/video/${req.files["video"][0].filename}`
        : null;
      const imgPath = req.files["img"]
        ? `uploads/image/${req.files["img"][0].filename}`
        : null;
...
router.post("/user-edit/:id", async (req, res) => {
  ...
  try {
    ...
    if (emailExists && emailExists._id.toString() !== userId.toString()) {
      return res.render("user/all-user", {
